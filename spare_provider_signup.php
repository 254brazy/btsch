...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    $dealers = trim($_POST['dealers']);
    $registration = trim($_POST['regstatus']);
    ...
    if (empty($firstname) || empty($secondname) || empty($company) || empty($role) || empty($email) || 
        ...
        empty($dealers) || empty($registration)) {
        ...
        $usercheck = $conn->prepare("SELECT username FROM sparecompany WHERE username=?");
        $usercheck->bind_param("s", $username);
        $usercheck->execute();
        $userresult = $usercheck->get_result();

        $emailcheck = $conn->prepare("SELECT email FROM sparecompany WHERE email=?");
        $emailcheck->bind_param("s", $email);
        $emailcheck->execute();
        $emailresult = $emailcheck->get_result();

        $telcheck = $conn->prepare("SELECT tel FROM sparecompany WHERE tel=?");
        $telcheck->bind_param("s", $tel);
        $telcheck->execute();
        $telresult = $telcheck->get_result();

        $companycheck = $conn->prepare("SELECT company FROM sparecompany WHERE company=?");
        ...
        if ($telresult->num_rows > 0) {
            ...
            $signup = $conn->prepare("INSERT INTO sparecompany (fname, sname, oname, company, role, email, tel, username, passhash, location, dealers, reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $signup->bind_param("ssssssssssss", $firstname, $secondname, $othername, $company, $role, $email, $tel, $username, $passwordtosave, $location, $dealers, $registration);
    ...
    <div class="container">
        <div class="content">
            ...
            <div class="form-section">
                <h2>Sign Up As Spare / Parts Provider Company</h2>
                
                <?php if (isset($message)) { echo $message; } ?>
                
                <form action="spare_provider_signup.php" method="post">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select id="role" name="role" required>
                                ...
                                <option value="founder" <?php echo (isset($_POST['role']) && $_POST['role'] == 'founder') ? 'selected' : ''; ?>>Founder</option>
                                <option value="staff" <?php echo (isset($_POST['role']) && $_POST['role'] == 'founder') ? 'selected' : ''; ?>>Staff</option>
                            </select>
                        </div>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label for="dealers">Spare/ Parts Class *</label>
                            <select id="dealers" name="dealers" required>
                                <option value="">-- Select Your Parts Dealings --</option>
                                <option value="electronics" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'electronics') ? 'selected' : ''; ?>>electronics</option>
                                <option value="mobile phones" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'motors') ? 'selected' : ''; ?>>Mobiles Phones</option>
                                <option value="computers" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'electrical') ? 'selected' : ''; ?>>Computers</option>
                                <option value="motorcycle" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'textile') ? 'selected' : ''; ?>>Motorcycle</option>
                                <option value="plumbing" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'furniture') ? 'selected' : ''; ?>>Plumbing</option>
                                <option value="construction" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Construction</option>
                                <option value="bicycles" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Bicycles</option>
                                <option value="electricals" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Electricals</option>
                                <option value="textile" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Textile</option>
                                <option value="foods" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Food & Bevarage</option>
                                <option value="furniture" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Furniture</option>
                                <option value="plastics" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Home Accessories</option>
                                
                            </select>
                        </div>
                        
                        <div class="form-group">
                            <label for="regstatus">Company Registration Status *</label>
                            <select id="regstatus" name="regstatus" required>
                                <option value="registered" <?php echo (isset($_POST['regstatus']) && $_POST['regstatus'] == 'registered') ? 'selected' : ''; ?>>Legally Registered</option>
                                <option value="unregistered" <?php echo (isset($_POST['regstatus']) && $_POST['regstatus'] == 'unregistered') ? 'selected' : ''; ?>>Not Registered</option>
                            </select>
                        </div>
                    </div>
                    
                    <div class="action-buttons">
                        <button type="submit" name="register" class="btn">Register Company</button>
        ...
        <div class="alternative-signup">
            ...
            <div class="signup-options">
                <a href="company_signup.php" class="btn btn-accent">Technician</a>
