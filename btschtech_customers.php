...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addstaff'])){
    ...
    $type=htmlspecialchars(trim($_POST['role']));
    ...
    if(empty($fname) || empty($sname)  || empty($companyid) || 
       empty($companyid)||empty($status) || empty($email) || empty($tel) || empty($type) || empty($branch ) || empty($password) || empty($confirm) ||empty($username))
       {
        ...
        $customercheck = $conn->prepare("SELECT username FROM customers WHERE username=?");
        $customercheck->bind_param("s", $username);
        $customercheck->execute();
        $userresult = $customercheck->get_result();
        ...
        if ($telresult->num_rows > 0) {
            ...
            // Insert branch
            $insert_customer = $conn->prepare("INSERT INTO customers (status,role,created_by,branch_id,fname,sname,oname,username,company_id,company,email,tel,passhash,branch) VALUES (?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)");
            $insert_customer->bind_param("sssissssisssss",$status,$type, $_SESSION['username'], $branchid,$fname,$sname,$oname,$username,$companyid,$company,$email,$tel,$hashpass,$branch);
            
            if($insert_customer->execute()) {
                $message = "<div class='message success'>Customer added successfully!</div>";
            } else {
                $message = "<div class='message error'>Error adding Customer: " . $conn->error . "</div>";
            }
        }
    }
}
...
if(isset($_GET['action']) && isset($_GET['customer_id'])) {
    ...
    switch($action) {
        case 'delete':
            $delete_stmt = $conn->prepare("DELETE FROM customers WHERE id = ? AND company = ?");
            $delete_stmt->bind_param("is", $customer_id, $_SESSION['company']);
            if($delete_stmt->execute()) {
                $message = "<div class='message success'> Customer deleted successfully</div>";
            } else {
                $message = "<div class='message error'>Error deleting Customer</div>";
            }
            break;
            
        case 'deactivate':
            $deactivate_stmt = $conn->prepare("UPDATE customers SET status = 'inactive' WHERE id = ? AND company = ?");
            $deactivate_stmt->bind_param("is", $customer_id, $_SESSION['company']);
            if($deactivate_stmt->execute()) {
                $message = "<div class='message success'>Customer deactivated</div>";
            }
            break;
            
        case 'activate':
            $activate_stmt = $conn->prepare("UPDATE customers SET status = 'active' WHERE id = ? AND company = ?");
            $activate_stmt->bind_param("is", $customer_id, $_SESSION['company']);
            if($activate_stmt->execute()) {
                $message = "<div class='message success'>Customer activated</div>";
            }
            break;
    }
}
...
$customer_query = $conn->prepare("SELECT id, fname,sname,tel,email,role,status,branch FROM customers WHERE company = ? ");
$customer_query->bind_param("s", $_SESSION['company']);
$customer_query->execute();
$customer_result = $customer_query->get_result();


// Get inactive staff for the company
$customer2_query = $conn->prepare("SELECT id, fname,sname,tel,email,role,status,created_at FROM customers WHERE company = ? AND status = 'inactive'");
$customer2_query->bind_param("s", $_SESSION['company']);
$customer2_query->execute();
$customer2_result = $customer2_query->get_result();
...
<head>
    ...
    <title><?php echo htmlspecialchars($_SESSION['company']); ?> -Customer Management</title>
    <style>
        :root {
            --primary: #2c3e50;
            --secondary: #3498db;
    ...
    <main>
        <aside>
            <div class="sidebar-header">
                ...
                <div class="user-branch">Customer Management</div>
            </div>
            <nav>
                <ul>
        ...
        <article>
            <div class="dashboard-header">
                ...
                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="openFormModal('addCustomersModal')">➕ Add Customer</button>
                    <a href="btschcustomers_branches.php" class="btn btn-warning">🎭 Manage Customer</a>
                </div>
            </div>
                ...
                <div class="section-header">
                    ...
                    <div class="action-buttons">
                        <span class="btn btn-sm" style="background: #f8f9fa;">
                            Total Active: <?php echo $customer_result->num_rows; ?> Customers
                            <br>
                            Total Inactive: <?php echo $customer2_result->num_rows; ?> Customers
                        </span>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php if($customer_result->num_rows > 0): ?>
                                    <?php while($customer = $customer_result->fetch_assoc()): ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($customer['fname']) .' ' . htmlspecialchars($customer['sname']) ; ?></td>
                                           ...
                                           <!-- <td><?php echo htmlspecialchars($customer['status']); ?></td> -->
                                            <td><?php echo htmlspecialchars($customer['branch']); ?></td>
                                            ...
                                            <td class="action-buttons">
                                                <a href="btschtechedit_customer.php?id=<?php echo $customer['id']; ?>" class="btn btn-sm btn-success">Edit</a>
                                                <a href="btschtechedit_customer.php?id=<?php echo $customer['id']; ?>" class="btn btn-sm btn-warning">Assign</a>
                                                <?php if($customer['status'] == 'active'): ?>
                                                    <a href="?action=deactivate&customer_id=<?php echo $customer['id']; ?>" class="btn btn-sm btn-warning">Deactivate</a>
                                                <?php else: ?>
                                                    <a href="?action=activate&customer_id=<?php echo $customer['id']; ?>" class="btn btn-sm btn-success">Activate</a>
                                                <?php endif; ?>
                                                <a href="?action=delete&customer_id=<?php echo $customer['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this Customer ?')">Delete</a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" style="text-align: center; padding: 20px;">
                                            No staff customers found. <button onclick="openFormModal('addCustomersModal')" class="btn btn-primary btn-sm">Add your first customer</button>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div id="addCustomersModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <span class="close" onclick="closeFormModal('addCustomersModal')">&times;</span>
            </div>
            <form method="POST" action="btschtech_customers.php">
                <div class="section-content">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            ...
                            <select name="role" id="role" required>
                               ...
                               <option value="services">Service Client</option>
                               <option value="sales">Sales Client</option>
                               <option value="enquiry">Enquiries Client</option>
                ...
                <div class="form-actions">
                    <button type="button" class="btn" onclick="closeFormModal('addCustomersModal')">Cancel</button>
                    <button type="submit" name="addstaff" class="btn btn-primary">Add Customers</button>
                </div>
            </form>
        </div>
    </div>
