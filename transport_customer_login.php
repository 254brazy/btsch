...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if (empty($username) || empty($tel) || empty($company) || empty($password)) {
        ...
        $user_check = $conn->prepare("SELECT id, username, passhash, fname, sname, role, company FROM transport_customers WHERE username = ? AND tel = ?  AND company = ?");
        ...
        if ($user_data && password_verify($password, $user_data['passhash'])) {
            ...
            // Update token
            $set_token = $conn->prepare("UPDATE transport_customers SET token = ?, last_login = NOW() WHERE username = ? AND company = ?");
            $set_token->bind_param("sss", $_SESSION['csrftoken'], $username, $company);
            $set_token->execute();
            
            header("Location: transport_customers_dashboard.php");
            exit();
        } else {
...
<head>
    ...
    <title>BTSCH - Transport Customer Login</title>
    <style>
        :root {
            --primary: #2c3e50;
            --secondary: #3498db;
...
<body>
    <div class="login-portal">
        <div class="portal-header">
            ...
            <p class="portal-subtitle">BrazyTech Supply Chain Hub - Transport & Logistics Customer Access</p>
        </div>
        
        <div class="portal-content">
            <!-- Login Section -->
            <div class="login-section">
                <h2 class="section-title">Transport & Logistics Customer Login</h2>
                <p style="color: #666; margin-bottom: 25px;">Enter your Transport & Logistics customer credentials to access the customer dashboard</p>
                ...
                <div style="margin-top: 25px; padding: 15px; background: #f8f9fa; border-radius: 8px;">
                    <p style="font-size: 0.9rem; color: #666; margin: 0;">
                        <strong>Note:</strong> This portal is for registered Transport & logistics Customers only.
                    </p>
                </div>
            </div>
            ...
            <div class="portal-links">
                ...
                <div style="margin-top: 30px; padding: 20px; background: white; border-radius: 10px; border-left: 4px solid var(--success);">
                    ...
                    <p style="font-size: 0.9rem; color: #666; margin-bottom: 15px;">
                        Register As Transport & Logistics Customer to join our supply chain network
                    </p>
                    <a href="transport_customer_signup.php" style="display: inline-block; padding: 10px 20px; background: var(--success); color: white; text-decoration: none; border-radius: 6px; font-weight: 600;">
                        Register As Transport & Logistics Customer
                    </a>
                </div>
            </div>
        </div>
