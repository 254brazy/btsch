...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addstaff'])){
    // Set form variables
    $servicename = htmlspecialchars(trim($_POST['name']));
    $serviceprice=htmlspecialchars(trim($_POST['price']));
    $currency = htmlspecialchars(trim($_POST['currency']));
    $status=htmlspecialchars(trim($_POST['status']));
    $companyid = htmlspecialchars(trim($_POST['companyid']));
    $company = htmlspecialchars(trim($_POST['company']));
    $branchservices=htmlspecialchars(trim($_POST['branch']));
   
    // Validation
    if(empty($servicename) || empty($serviceprice)  || empty($company) || 
       empty($companyid)||empty($currency)|| empty($branchservices) )
       {
        ...
        $servicecheck = $conn->prepare("SELECT service_name FROM techservices WHERE service_name=?");
        $servicecheck->bind_param("s", $servicename);
        ...
        if ($serviceresult->num_rows > 0) {
            $message = "<div class='message error'>Error: Service already Present</div>";
        }
        else {
             

            $branch_query = $conn->prepare("SELECT branch_name FROM branches WHERE company = ?");
            ...
            // Insert branch
            $insert_service = $conn->prepare("INSERT INTO techservices (service_name,service_price,company_id,company,branch,service_status,currency) VALUES (?,?,?, ?, ?, ?,?)");
            $insert_service->bind_param("siissss",$servicename,$serviceprice,$companyid,$company,$branchservices,$status,$currency);
            
            if($insert_service->execute()) {
                $message = "<div class='message success'>Service added successfully!</div>";
            } else {
...
if(isset($_GET['action']) && isset($_GET['service_id'])) {
    ...
    switch($action) {
        case 'delete':
            $delete_stmt = $conn->prepare("DELETE FROM techservices WHERE id = ?  AND company=?");
            $delete_stmt->bind_param("is", $service_id,$_SESSION['company']);
            if($delete_stmt->execute()) {
                $message = "<div class='message success'>  deleted successfully</div>";
            } else {
                $message = "<div class='message error'>Error deleting Service</div>";
            }
            break;
            
        case 'deactivate':
            $deactivate_stmt = $conn->prepare("UPDATE techservices SET service_status = 'inactive' WHERE id = ? AND company = ? ");
            $deactivate_stmt->bind_param("is", $service_id, $_SESSION['company']);
            if($deactivate_stmt->execute()) {
                $message = "<div class='message success'>Service Deactivated </div>";
            }
            break;
            
        case 'activate':
            $activate_stmt = $conn->prepare("UPDATE techservices SET service_status = 'active' WHERE id = ? AND company = ?");
            $activate_stmt->bind_param("is", $service_id, $_SESSION['company']);
            if($activate_stmt->execute()) {
                $message = "<div class='message success'>Service activated</div>";
            }
            break;
    }
}
...
$service_query = $conn->prepare("SELECT id,service_name,service_price,currency,service_status,branch,created_at FROM techservices WHERE company = ? ");
$service_query->bind_param("s", $_SESSION['company']);
$service_query->execute();
$service_result = $service_query->get_result();


// Get inactive staff for the company
$service2_query = $conn->prepare("SELECT id,service_name,service_price,currency,service_status,branch,created_at FROM techservices WHERE company = ? AND service_status = 'inactive'");
$service2_query->bind_param("s", $_SESSION['company']);
$service2_query->execute();
$service2_result = $service2_query->get_result();
...
<head>
    ...
    <title><?php echo htmlspecialchars($_SESSION['company']); ?> -Services Management</title>
    <style>
        :root {
            --primary: #2c3e50;
            --secondary: #3498db;
...
<body>
    ...
    <main>
        <aside>
            <div class="sidebar-header">
                ...
                <div class="user-branch">Service Management</div>
            </div>
            <nav>
                <ul>
        ...
        <article>
            <div class="dashboard-header">
                ...
                <div class="action-buttons">
                    <button class="btn btn-primary" onclick="openFormModal('addServicesModal')">➕ Add Services</button>
                ...
                <div class="section-header">
                    ...
                    <div class="action-buttons">
                        <span class="btn btn-sm" style="background: #f8f9fa;">
                            Total Active: <?php echo $service_result->num_rows; ?> Services
                            <br>
                            Total Inactive: <?php echo $service2_result->num_rows; ?> Services
                        </span>
                    </div>
                </div>
                                ...
                                <?php if($service_result->num_rows > 0): ?>
                                    <?php while($service = $service_result->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($service['currency']) .' ' . htmlspecialchars($service['service_price']) ; ?></td>
                                            <td><?php echo htmlspecialchars($service['service_status']); ?></td>
                                            <td><?php echo htmlspecialchars($service['branch']) . ' Branch'; ?></td>
                                            ...
                                            <td class="action-buttons">
                                                <a href="btschtechedit_services.php?id=<?php echo $service['id']; ?>" class="btn btn-sm btn-success">Edit</a>
                                                <a href="btschtechedit_services.php?id=<?php echo $service['id']; ?>" class="btn btn-sm btn-warning">Assign</a>
                                                <?php if($service['service_status'] == 'active'): ?>
                                                    <a href="?action=deactivate&service_id=<?php echo $service['id']; ?>" class="btn btn-sm btn-warning">Deactivate</a>
                                                <?php else: ?>
                                                    <a href="?action=activate&service_id=<?php echo $service['id']; ?>" class="btn btn-sm btn-success">Activate</a>
                                                <?php endif; ?>
                                                <a href="?action=delete&service_id=<?php echo $service['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this Services ?')">Delete</a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" style="text-align: center; padding: 20px;">
                                            No services found. <button onclick="openFormModal('addServicesModal')" class="btn btn-primary btn-sm">Add your first Service</button>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <div id="addServicesModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="btschtech_services.php">
                <div class="section-content">
                    <div class="form-grid">
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" name="addstaff" class="btn btn-primary">Add Service</button>
                </div>
            </form>
        </div>
    </div>
