...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    $branch=trim($_POST['mainbranch']);

    // Check empty fields
    if (empty($firstname) || empty($secondname) || empty($company) || empty($role) || empty($email) || 
        empty($tel) || empty($username) || empty($password) || empty($confirm) || empty($location) || empty($branch) ||
        ...
        $telcheck = $conn->prepare("SELECT tel FROM users WHERE tel=?");
        $telcheck->bind_param("s", $tel);
        $telcheck->execute();
        $telresult = $telcheck->get_result();

        $companycheck = $conn->prepare("SELECT company FROM users WHERE company=?");
        ...
        if ($telresult->num_rows > 0) {
            ...
            $signup = $conn->prepare("INSERT INTO users (fname, sname, oname, company, role, email, tel, username, passhash, location, dealers, reg,branch) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $signup->bind_param("sssssssssssss", $firstname, $secondname, $othername, $company, $role, $email, $tel, $username, $passwordtosave, $location, $dealers, $registration,$branch);
...
<body>
    ...
    <div class="container">
        <div class="content">
            ...
            <div class="form-section">
                <h2>Sign Up As Service Provider Company</h2>
                ...
                <form action="company_signup.php" method="post">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label for="location">Main Branch Name *</label>
                            <input type="text" id="mainbranch" name="mainbranch" value="<?php echo isset($_POST['mainbranch']) ? htmlspecialchars($_POST['location']) : ''; ?>" required placeholder="Main Branch">
                        </div>
                        
                        <div class="form-group">
                            <label for="dealers">Specialization *</label>
                            <select id="dealers" name="dealers" required>
                                ...
                                <option value="motors" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'motors') ? 'selected' : ''; ?>>Motors</option>
                                <option value="electrical" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'electrical') ? 'selected' : ''; ?>>Electrical</option>
                                <option value="textile" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'textile') ? 'selected' : ''; ?>>Textile</option>
                                <option value="furniture" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'furniture') ? 'selected' : ''; ?>>Furniture and Wood Work</option>
                                <option value="beuty" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Beauty and Hairdressing</option>
                                <option value="print" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'print') ? 'selected' : ''; ?>>Print Services</option>
                                <option value="plumbing" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'plumbing') ? 'selected' : ''; ?>>Plumbing and Water Works</option>
                                <option value="metal works" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'metal works') ? 'selected' : ''; ?>>Metal Work and Fabrication</option>
                            </select>
                        </div>
    ...
    <footer>
        <div class="container">
            <p>&copy; <?php echo date('Y'); ?> Brazy-Tech Link. All rights reserved.</p>
        </div>
    </footer>
</body>
</html>
