...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    if (empty($firstname) || empty($secondname) || empty($company) || empty($role) || empty($email) || 
        ...
        $usercheck = $conn->prepare("SELECT username FROM transportcompany WHERE username=?");
        $usercheck->bind_param("s", $username);
        $usercheck->execute();
        $userresult = $usercheck->get_result();

        $emailcheck = $conn->prepare("SELECT email FROM transportcompany WHERE email=?");
        $emailcheck->bind_param("s", $email);
        $emailcheck->execute();
        $emailresult = $emailcheck->get_result();

        $telcheck = $conn->prepare("SELECT tel FROM transportcompany WHERE tel=?");
        $telcheck->bind_param("s", $tel);
        $telcheck->execute();
        $telresult = $telcheck->get_result();

        $companycheck = $conn->prepare("SELECT company FROM transportcompany WHERE company=?");
        ...
        if ($telresult->num_rows > 0) {
            ...
            $signup = $conn->prepare("INSERT INTO transportcompany (fname, sname, oname, company, role, email, tel, username, passhash, location, dealers, reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    <div class="container">
        <div class="content">
            ...
            <div class="form-section">
                <h2>Sign Up As Transport Provider Company</h2>
                
                <?php if (isset($message)) { echo $message; } ?>
                
                <form action="transport_provider_signup.php" method="post">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select id="role" name="role" required>
                                ...
                                <option value="motorist" <?php echo (isset($_POST['role']) && $_POST['role'] == 'founder') ? 'selected' : ''; ?>>Motorist</option>
                            </select>
                        </div>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label for="dealers">Transport Mode *</label>
                            <select id="dealers" name="dealers" required>
                                <option value="">-- Select Your Transport Mode --</option>
                                <option value="Motorcycle" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'electronics') ? 'selected' : ''; ?>>Motorcycle</option>
                                <option value="Car" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'motors') ? 'selected' : ''; ?>>Car</option>
                                <option value="Van" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'electrical') ? 'selected' : ''; ?>>Van</option>
                                <option value="Trailer" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'textile') ? 'selected' : ''; ?>>Trailer</option>
                                <option value="Truck" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'furniture') ? 'selected' : ''; ?>>Truck</option>
                                <option value="Motorcycle" <?php echo (isset($_POST['dealers']) && $_POST['dealers'] == 'beuty') ? 'selected' : ''; ?>>Motorcycle</option>
        ...
        <div class="alternative-signup">
            ...
            <div class="signup-options">
                ...
                <a href="company.php" class="btn btn-accent">Technician</a>
